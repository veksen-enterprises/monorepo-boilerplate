...
- `.claude/skills/` bundles third-party skills vendored via [`npx skills`](https://github.com/vercel-labs/skills) from `mattpocock/skills` (engineering/workflow), `emilkowalski/skills` (design/motion), and `jakubkrehel/skills` (`better-colors`, `better-typography`, `better-ui` — wired into `/design-review`). Sources and hashes are tracked in `skills-lock.json`.
