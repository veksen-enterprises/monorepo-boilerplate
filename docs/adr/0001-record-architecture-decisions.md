...
We keep a log of the architecturally significant decisions on this project, one Markdown
file per decision, in `docs/adr/`.
...
sometimes reversed at cost. A short record written at the moment of the decision keeps the
_why_ next to the code, and lets a later reader change it deliberately instead of by
accident.
